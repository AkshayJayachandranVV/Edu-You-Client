...
const TutorChatInterface: React.FC<ChatInterfaceProps> = ({ selectedChat }) => {
  ...
  const { tutorname } = useSelector((state: RootState) => state.tutor);
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (selectedChat?.courseId) {
        console.log(
          "Cleanup: handling exit for selectedChat room:",
  ...
  useEffect(() => {
    ...
    if (selectedChat?.courseId) {
      ...
      if (storedRoomId) {
        console.log(
          "No selectedChat, handling exit for stored room:",
          storedRoomId
        );
        handleChatExit(storedRoomId);
      }
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Emit typing status only if selectedChat and courseId are valid
    emitTyping(true, selectedChat?.courseId || "", tutorname);
    ...
    setTypingTimeout(
      setTimeout(() => {
        emitTyping(false, selectedChat?.courseId || "", tutorname); // Emit stop typing after delay
      }, 1000)
    );
  };
  ...
  const fetchChat = async () => {
    try {
      ...
      const fetchedMessages: any[] = response.data; // Ensure this matches your API response structure

      console.log("Fetched messages:", fetchedMessages);

      const formattedMessages = fetchedMessages.map(
        (message: any, index: number): Message => ({
          id: index + 1,
  ...
  useEffect(() => {
    const handleMediaReceive = (mediaData: any) => {
      ...
      setDisplayedMessages((prev: any) => [...prev.slice(-5), newMediaMsg]); // Update displayed messages
    };

    SocketService.onReceiveMedia(handleMediaReceive);  
    SocketService.onTutorReceiveMedia(handleMediaReceive);
    return () => {
  ...
  useEffect(() => {
    if (selectedChat) {
      ...
      const messageHandler = (message: any) => {
        ...
        setDisplayedMessages((prev: any) => [...prev.slice(-5), newMsg]); // Update displayed messages
      };

      const typingHandler = (typingStatus: any) => setIsTyping(typingStatus); // Specify typingStatus as boolean
  ...
  const handleSendMessage = async () => {
    try {
      // Ensure `selectedChat` and `userId` are not null or undefined
      if (!selectedChat?.courseId || !userId) {
      ...
      if (selectedFile) {
        ...
        if (uploadResponse) {
          ...
          await SocketService.sendTutorMessage({
        ...
        // Emit typing stopped
        SocketService.emitTyping(false, selectedChat.courseId, "You");
      }
    } catch (error) {
    ...
    if (selectedFile) {
      try {
        ...
        if (uploadResponse) {
          ...
          // Send the media message through the socket
          SocketService.sendTutorMedia({
            roomId: selectedChat?.courseId || "",
            senderId: userId || "",
            mediaUrl: url,
            s3Key: key,
          ...
          // Update the state with the new media message
          setMessages((prevMessages) => [
            ...prevMessages,
            {
              id: prevMessages.length + 1,
              messageId: key, // Assuming `key` is unique and can act as a messageId
              mediaUrl: url,
              mediaType,
              sender: "You",
              time: new Date().toLocaleTimeString(),
              isSender: true,
              username: "You",
              isRead: false, // Adding the missing `isRead` property
            },
          ]);
        ...
        <div className="flex items-center space-x-4">
          ...
          {selectedChat && ( // Only render the button if selectedChat is truthy
            <button
              ...
              className="w-full px-4 py-2 bg-green-500 text-white rounded-md font-semibold hover:bg-green-600 transition duration-200"
            >
              Join Live
            </button>
          )}
