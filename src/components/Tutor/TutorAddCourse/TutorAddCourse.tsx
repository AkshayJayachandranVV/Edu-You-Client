...
const AddCourse: React.FC<AddCourseProps> = ({ onNext }) => {
    ...
    try {
      ...
      const response = await axios.get(
        tutorEndpoints.getPresignedUrlForUpload,
        {
          params: {
            filename: fileName,
            fileType: uploadedFile!.type.startsWith("image")
              ? "image"
              : "video",
          },
        }
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-4xl shadow-lg rounded-lg p-8"
        style={{ backgroundColor: "#1b2532", marginTop: "20px" }}
      >
        ...
        <form className="space-y-8" onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              <Controller
                ...
                rules={{
                  ...
                  min: { value: 0, message: "Price must be a positive number" },
                  validate: (value) =>
                    value < getValues("coursePrice") ||
                    "Discount price must be less than course price",
                }}
                render={({ field }) => (
                  <input
                    {...field}
          ...
          <div className="flex flex-col gap-4">
            ...
            />
            {errors.demoURL && (
              <span className="text-red-500">{errors.demoURL.message}</span>
            )}
          </div>
